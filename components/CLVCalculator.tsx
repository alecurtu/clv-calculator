  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl md:text-3xl font-bold tracking-tight">NFL Closing Line Value (CLV) Calculator</h1>
        <span className={`px-2 py-0.5 rounded-full text-xs ${clvIsPositive ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-white rounded-2xl shadow p-6">
        <div className="flex flex-col gap-2">
          ...
          <select value={side} onChange={(e) => setSide(e.target.value as 'Over' | 'Under')} className="h-10 rounded-md border px-3">
        ...
        <div className="flex flex-col gap-2">
          <label className="text-sm text-slate-600">Original Line (total)</label>
          <input type="number" value={originalLine} onChange={(e) => setOriginalLine(parseFloat(e.target.value))} className="h-10 rounded-md border px-3" />
        </div>

        <div className="flex flex-col gap-2">
          <label className="text-sm text-slate-600">Original Odds (American)</label>
          <input type="number" value={originalOdds} onChange={(e) => setOriginalOdds(parseInt(e.target.value))} className="h-10 rounded-md border px-3" />
        </div>

        <div className="flex flex-col gap-2">
          <label className="text-sm text-slate-600">Closing Line (total)</label>
          <input type="number" value={closingLine} onChange={(e) => setClosingLine(parseFloat(e.target.value))} className="h-10 rounded-md border px-3" />
        </div>

        <div className="flex flex-col gap-2">
          <label className="text-sm text-slate-600">Closing Odds (American)</label>
          <input type="number" value={closingOdds} onChange={(e) => setClosingOdds(parseInt(e.target.value))} className="h-10 rounded-md border px-3" />
        </div>

        <div className="flex flex-col gap-2">
          <label className="text-sm text-slate-600">Cents per Half-Point</label>
        ...
        <div className="flex flex-col gap-2 md:col-span-3">
          <label className="text-sm text-slate-600">Market Rounding</label>
        ...
        <div className="bg-white rounded-2xl shadow p-6 flex flex-col gap-4">
          ...
          <div className="text-sm text-slate-600">
            ...
            <ul className="list-disc ml-5 mt-1">
              ...
              <li>Use market rounding to mimic typical book prints (e.g., +120 instead of +122).</li>
            </ul>
          </div>
        </div>
      </div>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        <button className="h-9 rounded-md px-3 border" onClick={() => {
      ...
      <p className="mt-6 text-xs text-slate-500">
        This tool is for informational purposes only and not financial advice. Odds conversion uses standard American ↔ implied probability formulas. The "cents per half-point" model is a linear approximation; real markets vary by key numbers, vig, and book microstructure.
      </p>
    </div>
  );
}
