...
export default function CLVCalculator() {
  ...
  const chip = clvIsPositive ? 'bg-emerald-500/15 text-emerald-400 shadow-neon-emerald' : 'bg-fuchsia-500/15 text-fuchsia-300 shadow-neon-fuchsia';
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl md:text-3xl font-bold tracking-tight">NFL Closing Line Value (CLV)</h2>
        <span className={`px-2 py-0.5 rounded-full text-xs ${chip}`}>{clvIsPositive ? 'Positive CLV' : 'Negative CLV'}</span>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 glass rounded-2xl border border-slate-200/60 dark:border-white/10 p-6 neon-border">
        <div className="flex flex-col gap-2">
          <label className="text-sm text-slate-600 dark:text-slate-300">Side</label>
          <select value={side} onChange={(e) => setSide(e.target.value as 'Over' | 'Under')} className="h-10 rounded-md border px-3 bg-white/70 dark:bg-white/5">
        ...
        <div className="flex flex-col gap-2">
          ...
          <input type="number" value={originalLine} onChange={(e) => setOriginalLine(parseFloat(e.target.value))} className="h-10 rounded-md border px-3 bg-white/70 dark:bg-white/5" />
        </div>
        <div className="flex flex-col gap-2">
          <label className="text-sm text-slate-600 dark:text-slate-300">Original Odds (American)</label>
          <input type="number" value={originalOdds} onChange={(e) => setOriginalOdds(parseInt(e.target.value))} className="h-10 rounded-md border px-3 bg-white/70 dark:bg-white/5" />
        </div>
        <div className="flex flex-col gap-2">
          <label className="text-sm text-slate-600 dark:text-slate-300">Closing Line (total)</label>
          <input type="number" value={closingLine} onChange={(e) => setClosingLine(parseFloat(e.target.value))} className="h-10 rounded-md border px-3 bg-white/70 dark:bg-white/5" />
        </div>
        <div className="flex flex-col gap-2">
          <label className="text-sm text-slate-600 dark:text-slate-300">Closing Odds (American)</label>
          <input type="number" value={closingOdds} onChange={(e) => setClosingOdds(parseInt(e.target.value))} className="h-10 rounded-md border px-3 bg-white/70 dark:bg-white/5" />
        </div>
        <div className="flex flex-col gap-2">
        ...
        <div className="flex flex-col gap-2 md:col-span-3">
          ...
          <div className="flex gap-2">
            <button onClick={() => setRoundMode('exact')} className={`h-9 rounded-md px-3 border bg-white/70 dark:bg-white/5 ${roundMode==='exact' ? 'ring-2 ring-[var(--ring)] text-slate-900 dark:text-white' : ''}`}>Exact (no-vig)</button>
            <button onClick={() => setRoundMode('tick5')} className={`h-9 rounded-md px-3 border bg-white/70 dark:bg-white/5 ${roundMode==='tick5' ? 'ring-2 ring-[var(--ring)] text-slate-900 dark:text-white' : ''}`}>Nearest 5¢</button>
            <button onClick={() => setRoundMode('tick10')} className={`h-9 rounded-md px-3 border bg-white/70 dark:bg-white/5 ${roundMode==='tick10' ? 'ring-2 ring-[var(--ring)] text-slate-900 dark:text-white' : ''}`}>Nearest 10¢</button>
          </div>
        </div>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="glass rounded-2xl border border-slate-200/60 dark:border-white/10 p-6 neon-border flex flex-col gap-4">
        ...
        <div className="glass rounded-2xl border border-slate-200/60 dark:border-white/10 p-6 neon-border flex flex-col gap-4">
          ...
          <Stat label="Edge (Probability Points)" value={`${(results.edgeProb * 100).toFixed(2)} pp`} helper="Positive = you beat the close" />
        </div>
        <div className="glass rounded-2xl border border-slate-200/60 dark:border-white/10 p-6 neon-border flex flex-col gap-4">
          ...
          <div className="text-sm text-slate-600 dark:text-slate-400">
            <ul className="list-disc ml-5 mt-1">
              <li>Adjust cents/half-point for key totals (e.g., 41–44, 47, 51 can be pricier).</li>
              <li>Use market rounding to mimic typical prints (e.g., +120 instead of +122).</li>
            </ul>
          </div>
        </div>
      </div>
      <div className="mt-4 flex flex-wrap gap-2">
        <button className="h-9 rounded-md px-3 border bg-white/70 dark:bg-white/5" onClick={() => {
          setSide('Under'); setOriginalLine(46); setOriginalOdds(-102); setClosingLine(47); setClosingOdds(-107); setCentsPerHalfPoint(10); setRoundMode('exact');
        }}>Load Under Demo</button>
        <button className="h-9 rounded-md px-3 border bg-white/70 dark:bg-white/5" onClick={() => {
          setSide('Over'); setOriginalLine(46); setOriginalOdds(-107); setClosingLine(47); setClosingOdds(-106); setCentsPerHalfPoint(10); setRoundMode('tick5');
        }}>Load Over Demo</button>
        <button className="h-9 rounded-md px-3 border bg-white/70 dark:bg-white/5" onClick={() => {
          setSide('Under'); setOriginalLine(43); setOriginalOdds(-107); setClosingLine(44); setClosingOdds(-110); setCentsPerHalfPoint(10); setRoundMode('tick5');
        }}>Load Example #3</button>
      </div>
      <p className="mt-6 text-xs text-slate-500 dark:text-slate-400">
        This tool is for informational purposes only. The model is a linear approximation; real markets vary by key numbers, vig, and book microstructure.
      </p>
    </div>
  );
}
